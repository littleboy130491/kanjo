...
<x-layout :locale="$locale" :title="$invoice->document_number" :company="$company" :pdf-mode="$pdfMode" :slug="$slug"
    ...
    <section class="border-b pb-4">
        <div class="flex items-start justify-between">
            ...
            <div class="text-right text-sm">
                ...
                <span
                    class="inline-block rounded-full px-3 py-1 text-xs font-semibold {{ $badgeColor }}">{{ $invoice->payment_status->getLabel() }}</span>
            </div>
        </div>
    </section>
    ...
    <section>
        ...
        <p>{{ $invoice->client_company }} — {{ $invoice->client_name }}</p>
        <p>{{ $invoice->client_email }} | {{ $invoice->client_phone }}</p>
    </section>
