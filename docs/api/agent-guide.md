...
`title` / `party_identification` / `subject` / `content` / `signature` `default` copy SPK defaults and resolve placeholders once. Empty stored `title` or `party_identification` falls back to the auto-generated cover/party block, while empty `signature` falls back to the generated signing area:
